// still bugs
// bug 1, if text contain - then - is NOT encoded
...
// from [link-to-text-fragment  /fragment-generation-utils.js - Github](https://github.com/GoogleChromeLabs/link-to-text-fragment/blob/main/fragment-generation-utils.js)
/**
...
if (typeof goog !== 'undefined') {
  ...
  // clang-format on
        ...
        function getSelectionContent() {
            ...
            if (window.getSelection && (windowSelected=window.getSelection()) ) {
                selection = windowSelected;
            } else if (document.selection && document.selection.type != "Control" && 
                                    (documentSelected = document.selection.createRange()) ) {
                selection = documentSelected;
            }
            return selection;
        }
        ...
        function getURL() {
            if (isThatSite(site, "bilibili.com") && isThatPath(location.pathname, '/video'))
                return `${location.origin}${location.pathname}${getJumpToPart(getSelectionContent())}`;
            return `${location.origin}${location.pathname}${location.search}${getJumpToPart(getSelectionContent())}`;
        }

        // from [How to copy link to highlight using chrome API - Stack Overflow](https://stackoverflow.com/questions/71806679/how-to-copy-link-to-highlight-using-chrome-api)
        function getJumpToPart(selection) {
            const genFragResult = generateFragment(selection);
            if (genFragResult.status === 0) {
              const fragment = genFragResult.fragment;
              ...
              var part = `#:~:text=${prefix}${textStart}${textEnd}${suffix}`;
              return part;
            } else {
              return '';
            }
